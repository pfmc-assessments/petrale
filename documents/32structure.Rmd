...
The structure of the base model was selected to balance model realism and parsimony. A large number of alternate model formulations were evaluated during the assessment process. Structural choices were generally made to be as objective as possible, and follow generally accepted methods of approaching similar modeling problems and data issues. The relative effect on assessment results of each of these choices is often unknown; however, extensive efforts were made to evaluate effects of structural choices on model output prior to selecting the base model.

Prior to arriving at the base model, an extensive evaluation of model structure was performed. We explored retaining the seasonal fleet model of the previous assessment versus annual two fleet models. We also explored a single annual fleet model (combining North and South fleets), as well as two-area model, with a single set of goth parameters estimated as well as separate growth parameters estimated for each area. These models produced very similar results, yet the annual two-fleet model was found to be the most appropriate for this assessment, as it resulted in best fit to data and parameters estimates the most consistent with current knowledge about `r spp`, while accounting for the difference in history of removals among North and South. As mentioned earlier, in the past, the separation of North and South fisheries into the Winter and Summer fleets were primarily motivated by the using the fishery CPUE indices form Winter fisheries, targeting `r spp` spawning aggregations. With removal of CPUE indices from the model, separation into Winter and Summer fleets was no longer needed. Also, Winter and Summer fleets selectivity curves were very similar within respective fisheries. Combining Winter and Summer fleets within North and South fisheries yielded very similar results (Figure \ref{fig:bridge2-ssb}). Combining seasonal fleets also removed uncertainty associated with separating historical annual catches by season (since fishing year for Winter fleet was defined from November of previous year through the February of the current year). Finally, using annual catches puts assessment in alignment with management system, which operates on a calendar year basis.  

We extensively evaluated structure of \gls{s-tri} and how to best incorporate  \gls{s-tri} index. Our exploration led us to switching to a single index time series (instead of separating it into two indices for early and late survey periods), as it provided a longer historical survey trend and simplified the structure of the assessment model, without impacting model fits and results (Figure \ref{fig:bridge1-ssb}).

We experimented with blocking the selectivity curves in commercial fleets to enable reflection of changes associated with management measures and arrive to model that would allow to best fit to data. Specifically, we implemented blocking for the period after IFQ fishery began, that allowed to reflect changes in discard practices. We also re-evaluation early blocking on retention parameters to ensure that estimation of early discard is informed by sufficient amount of data. 

The quality of data available was also thoroughly evaluated, that included data sources (NWFSC Slope survey and AFSC Slope survey) and also elements within eaxh source (set of ages). 
