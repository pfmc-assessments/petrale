...
Prior to arriving at the base model, an extensive evaluation of model structure was performed. We explored retaining the four-fleet model with seasonal fisheries of the previous assessment versus two annual fleets models. We also explored a single annual fleet model (combining North and South fleets), as well as two-area model, with a single set of growth parameters estimated as well as separate growth parameters estimated for each area. These models produced very similar results, yet the annual two-fleet model was found to be the most appropriate for this assessment. The selected formulation allowed to simplify the model structure as it resulted in best fit to data and parameters estimates, which are the most consistent with current knowledge about `r spp`, while accounting for the difference in history of removals among North and South. The simplified model also allowed to substantially reduce model run time (from several hours to eighteen minutes) as well as improve final gradient. 

As mentioned earlier, the separation of North and South fisheries into the Winter and Summer fleets in past assessments were primarily motivated by the using the fishery CPUE indices form Winter fisheries, targeting `r spp` spawning aggregations. With removal of CPUE indices from the model, separation into Winter and Summer fleets was no longer needed. Also, Winter and Summer fleets selectivity curves were very similar within respective fisheries. Combining Winter and Summer fleets within North and South fisheries yielded very similar results (Figure \ref{fig:bridge2-ssb}). Combining seasonal fleets also removed uncertainty associated with separating historical annual catches by season (since fishing year for Winter fleet was defined from November of previous year through the February of the current year). Finally, using annual catches puts assessment in alignment with management system, which operates on a calendar year basis.  

Substantial amount of efforts was devoted to evaluation of the quality of data available for the assessment, and structural choices were made regarding whether and how specific sources or their components should be used in the model. This included decisions on filtering fisheries length and age composition data, treatment of survey indices, and incorporation on environmental indices in the model.

We also experimented with blocking the selectivity curves in fisheries fleets to enable reflection of changes associated with management measures and arrive to model that would allow to best fit to data. Specifically, we implemented blocking for the period after IFQ fishery began, that allowed to reflect changes in discard practices. We also re-evaluation early blocking on retention parameters to ensure that estimation of early discard is informed by sufficient amount of data. 
