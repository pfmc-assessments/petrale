...
table_parcounts <- function(
  ...
  if (is.character(dir) &&
    is.character(ctlfile) &&
    file.exists(file.path(dir, ctlfile))) {
    ctl <- SS_readctl(file.path(dir, ctlfile))
  } else {
    stop(
      "File not found:", file.path(dir, ctlfile),
      "Inputs 'dir' and 'ctlfile' need to provide the ",
      "the directory and filename for a control file."
    )
  }
