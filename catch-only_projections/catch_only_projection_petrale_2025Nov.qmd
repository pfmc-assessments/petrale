...
# check for bad ACL vs ABC
if (any(table$ACL > table$ABC + 0.1, na.rm = TRUE)) {
    cli::cli_alert_warning("ACL > ABC check table below.")
    print(table)
}
# check for bad ratios
...
# check for bad ratios in original model
...
# move 2026 ACL (which is really fixed catch from SSL) to the assumed catch column
...
wide_table3$"Actual & Assumed Removals"[wide_table3$Year %in% 2026:2028] <- 
  wide_table3$ACL[wide_table3$Year %in% 2026:2028]
# calculate 2026 ACL from ABC
# get 2026 stock status for use in 40-10 or 25-5 rule
D <- wide_table3$"Stock Status"[wide_table3$Year %in% 2026:2028]
#
adj <- rep(1.0, length(D))
# 40-10 rule for most species
if (model$btarg == 0.40) {
  adj[D < model$btarg] <- (4/3)*(D-0.1)/D
}
# 25-5 rule for flatfish
if (model$btarg == 0.25) {
  adj[D < model$btarg] <- (25/20)*(D-0.05)/D
}
# calculate ACL from adjustment * ABC
wide_table3$ACL[wide_table3$Year %in% 2026:2028] <- 
  adj * wide_table3$ABC[wide_table3$Year %in% 2026:2028]
