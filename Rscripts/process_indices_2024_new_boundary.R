...
# this is currently in the "tribal_UandA" branch
# https://github.com/pfmc-assessments/indexwc/compare/main...tribal_UandA
...
run_sdmtmb(
  ...
  anisotropy = TRUE,
  n_knots = 500,
  boundaries = boundaries_data[c(
    "Coastwide",
    "North of 46deg 53.3min",
    "South of 46deg 53.3min"
  )],
  spatiotemporal = list("iid", "iid")
)

# copy all files from temporary directory created by indexwc to a permanent directory
from_dir <- "petrale_sole/wcgbts/delta_lognormal"
to_dir <- "data-raw/wcgbts/delta_lognormal_with2024_and_UandA_boundary"

if (!dir.exists(to_dir)) {
  dir.create(to_dir, recursive = TRUE)
}
files_to_copy <- list.files(from_dir, full.names = TRUE)
file.copy(files_to_copy, to_dir, overwrite = TRUE, recursive = TRUE)


est_by_area <- read.csv(
  "data-raw/wcgbts/delta_lognormal_with2024_and_UandA_boundary/index/est_by_area.csv"
)
...
results_north |>
  ...
  geom_segment(
    aes(x = x1, y = y1, xend = x2, yend = y2, color = "mean of recent 5 obs"),
    lwd = 1,
    data = data.frame(
      x1 = 2018,
      x2 = 2024,
  ...
  ylab(expression(
    "Ratio of estimated biomass North of 46\u00B053.3' to coastwide biomass"
  )) +
  theme_bw()

ggsave(
  "data-raw/wcgbts/delta_lognormal_with2024_and_UandA_boundary/ratio_plot.png",
  width = 10,
  height = 7
)
...
results_coast |>
  ...
  dplyr::mutate(
    "Biomass North of 46\u00B053.3' (mt)" = round(results_north$est)
  ) |>
  dplyr::mutate(Ratio = round(results_north$ratio, 3)) |>
  knitr::kable(format = "html") |>
  writeLines(
    "data-raw/wcgbts/delta_lognormal_with2024_and_UandA_boundary/results_table.html"
  )

avg_5yr_coast <- results_coast |>
  dplyr::filter(year >= 2019) |>
  dplyr::pull(est) |>
  mean() |>
  round()

avg_5yr_north <- results_north |>
  dplyr::filter(year >= 2019) |>
  dplyr::pull(est) |>
  mean() |>
  round()

avg_coast <- results_coast |>
  dplyr::pull(est) |>
  mean() |>
  round()

avg_north <- results_north |>
  dplyr::pull(est) |>
  mean() |>
  round()

data.frame(
  "Average Coastwide Biomass (mt)" = c(avg_coast, avg_5yr_coast),
  "Average Biomass North of 46\u00B053.3' (mt)" = c(avg_north, avg_5yr_north),
  "Average Ratio" = c(
    mean_all,
    mean_recent5
  ),
  row.names = c("All years", "Recent 5 years")
) |>
  knitr::kable(format = "html") |>
  writeLines(
    "data-raw/wcgbts/delta_lognormal_with2024_and_UandA_boundary/averages_table.html"
  )

# remake index plot with fewer lines
p <- est_by_area |>
  dplyr::filter(
    area %in% c("Coastwide", "North of 46deg 53.3min", "South of 46deg 53.3min")
  ) |>
...
p +
  ...
  ggplot2::scale_colour_manual(
    values = c(
      "Coastwide" = "black",
      "North of 46deg 53.3min" = "blue",
      "South of 46deg 53.3min" = "red"
    )
  ) +
  expand_limits(y = 0) +
  scale_y_continuous(expand = c(0, 0))
ggsave(
  "data-raw/wcgbts/delta_lognormal_with2024_and_UandA_boundary/index_modified.png",
  width = 10,
  height = 7
)
