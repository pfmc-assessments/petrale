...
#
# MODIFIED 27 March 2026 to use new column names in the bds-colunn-naming branch of pacfintools
#
################################################################################################

# changes from orinal script (Rscripts/process_pacfin_bds.R):
# 1. change SOURCE_AGID to AGENCY_CODE
# 2. 
#
...
if (FALSE) {
  tictoc::tic()
  source("Rscripts/process_pacfin_bds_NEW_COLUMN_NAMES.R")
  tictoc::toc()
  # 280.06 sec elapsed
}
...
test <- Pdata$SAMPLE_TYPE != "S" | (Pdata$SAMPLE_TYPE <= 1986 & Pdata$AGENCY_CODE == "O")
# remove samples that are special projects after 1986 and from other agencies
Pdata2 <- Pdata %>%
  dplyr::filter(SAMPLE_TYPE != "S" | (SAMPLE_TYPE <= 1986 & AGENCY_CODE == "O"))
nrow(Pdata) - nrow(Pdata2)
# [1] 24536

# confirm that CalCOM ages got retained after cleaning
table(Pdata$Age, Pdata$AGENCY_CODE) %>% head()
#      C CalCOM    O    W
# 1    0      3   10    1
# 2    0     58   18   38
# 3   31    759  328  577
...
# California
Pdata$ageerr[!is.na(Pdata$Age) & Pdata$AGENCY_CODE %in% c("C", "CalCOM") & Pdata$agemethod == "S" & Pdata$fishyr < 1990] <- 8 # CAP pre-1990 surface

# There are middle years where there are U BB and S reads
Pdata$ageerr[!is.na(Pdata$Age) & Pdata$AGENCY_CODE %in% c("C", "CalCOM") & Pdata$fishyr >= 1985 & Pdata$fishyr <= 1991] <- 4 # CAP Combo
Pdata$ageerr[!is.na(Pdata$Age) & Pdata$AGENCY_CODE %in% c("C", "CalCOM") & Pdata$agemethod == "B" & Pdata$fishyr >= 1992] <- 2 # CAP BB

# Oregon
Pdata$ageerr[!is.na(Pdata$Age) & Pdata$AGENCY_CODE == "O" & Pdata$agemethod == "S" & Pdata$fishyr <= 1980] <- 8 # CAP pre-1990 surface
Pdata$ageerr[!is.na(Pdata$Age) & Pdata$AGENCY_CODE == "O" & Pdata$fishyr >= 1980 & Pdata$fishyr < 1999] <- 4 # CAP Combo
Pdata$ageerr[!is.na(Pdata$Age) & Pdata$AGENCY_CODE == "O" & Pdata$agemethod == "B" & Pdata$fishyr >= 1999] <- 2 # CAP BB
Pdata$ageerr[!is.na(Pdata$Age) & Pdata$AGENCY_CODE == "O" & Pdata$agemethod == "S" & Pdata$fishyr >= 1999] <- 3 # CAP Surface
Pdata$ageerr[!is.na(Pdata$Age) & Pdata$AGENCY_CODE == "O" & Pdata$fishyr >= 2007] <- 2 # CAP BB

# Washington
Pdata$ageerr[!is.na(Pdata$Age) & Pdata$AGENCY_CODE == "W" & Pdata$agemethod %in% c("S")] <- 6 # WDFW Surface
Pdata$ageerr[!is.na(Pdata$Age) & Pdata$AGENCY_CODE == "W" & Pdata$agemethod %in% c("U") & Pdata$fishyr < 2008] <- 6 # WDFW Surface
Pdata$ageerr[!is.na(Pdata$Age) & Pdata$AGENCY_CODE == "W" & Pdata$agemethod == "B"] <- 7 # WDFW BB
Pdata$ageerr[!is.na(Pdata$Age) & Pdata$AGENCY_CODE == "W" & Pdata$fishyr %in% c(2009, 2010)] <- 5 # WDFW Combo
...
test <- Pdata_exp1_annual %>%
  dplyr::filter(AGENCY_CODE == "CalCOM")
...
# look at expansions
Pdata_exp2_annual %>%
  ggplot(aes(x = Expansion_Factor_1_L, fill = AGENCY_CODE, colour = AGENCY_CODE)) +
...
Pdata_exp2_annual %>%
  ggplot(aes(x = Expansion_Factor_2, fill = AGENCY_CODE, colour = AGENCY_CODE)) +
  geom_histogram(alpha = 0.5, position = "stack")
...
Pdata_exp2_annual %>%
  ggplot(aes(x = Final_Sample_Size, fill = AGENCY_CODE, colour = AGENCY_CODE)) +
  geom_histogram(alpha = 0.5, position = "stack")
...
table(Pdata$AGENCY_CODE, Pdata$SEX)
#            F     M     U
# C      14880 34416  8272
...
Adata_exp2_annual %>%
  ggplot(aes(x = Expansion_Factor_1_A, fill = AGENCY_CODE, colour = AGENCY_CODE)) +
  geom_histogram(alpha = 0.5, position = "stack")
if (save) {
